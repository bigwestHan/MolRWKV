...
class RWKV(pl.LightningModule):
    def __init__(self, args):
        ...
        self.pro_lin=nn.Linear(args.pro_feat, args.n_embd)
        self.aff_lin=nn.Linear(1, args.n_embd)
    ...
    def forward(self, x,proteins=None, affinity=None):
        args = self.args
        ...
        if self.args.proteins:
            type_embd = self.type_emb(torch.zeros((b, 1), dtype = torch.long, device = x.device))
            proteins=self.pro_lin(proteins.unsqueeze(1))+type_embd
            if affinity.dim()==2:
                affinity=affinity.unsqueeze(-1)
            elif affinity.dim()==1:
                affinity=affinity.unsqueeze(-1).unsqueeze(-1)
            affinity=self.aff_lin(affinity)+type_embd
            x=torch.cat([proteins,affinity,x],dim=1)
        if args.dropout > 0:
            x = self.drop0(x)
...
class MyRWKV(pl.LightningModule):
    ...
    def forward(self, samples):
        ...
        x,proteins,affinity, y = samples["x"], samples["proteins"], samples["affinity"], samples["y"]
        x = self.rwkv(x,proteins,affinity) # 
        return x, y
    ...
    @torch.no_grad()
    def predict(self, input_points,proteins=None,affinity=None) -> list[int]:
        
        outputs = self.rwkv(input_points,proteins,affinity)
        return outputs
