...
class RWKV_Tmix_x060(MyModule):
    def __init__(self, args,n_layer, layer_id):
        super().__init__()
        self.args = args
...
class Block_d(nn.Module):
    def __init__(self, args, layer_id):
        ...
        self.att = RWKV_Tmix_x060(args,args.de_layer, layer_id)
        self.att1=CrossAttention(args.n_embd,args.head_size_divisor)
        self.ffn = RWKV_CMix_x060(args,args.de_layer, layer_id)

        if args.dropout > 0:
            self.drop0 = nn.Dropout(p = args.dropout)
            self.drop1 = nn.Dropout(p = args.dropout)
        
    def forward(self, x,en=None):
        if self.layer_id == 0:
            x = self.ln0(x)
        y=self.ln1(x)
        y=y.permute(0,2,1)
        ...
        if en is not None:
            x=x+self.att1(self.ln3(x),self.ln3(en))
...
class Block_e(nn.Module):
    def __init__(self, args, layer_id):
        ...
        if self.layer_id == 0:
            self.ln0 = nn.LayerNorm(args.n_embd)
        self.att = RWKV_Tmix_x060(args,args.en_layer, layer_id)
        self.ffn = RWKV_CMix_x060(args,args.en_layer, layer_id)
...
class RWKV(pl.LightningModule):
    def __init__(self, args):
        ...
        if args.num_props:
            self.p_lin=nn.Linear(1,args.n_embd)
        self.type_emb=nn.Embedding(2,args.n_embd)
        self.blocks_d = nn.ModuleList([Block_d(args, i) for i in range(args.de_layer)])
        self.blocks_e = nn.ModuleList([Block_e(args, i) for i in range(args.en_layer)])
    ...
    def forward(self, x, p=None, scaffold=None):
        args = self.args
        ...
        if self.args.scaffold:
            ...
            x=torch.cat((scaffold,x),dim=1)

        en=None
        if self.args.num_props:
            p = self.p_lin(p.unsqueeze(-1))
            if en is None:
                en=p
            else:
                en=torch.cat((p,en),dim=1)

        if en is not None:
            for block in self.blocks_e:
                if args.grad_cp == 1:
                    en = deepspeed.checkpointing.checkpoint(block, en)
                else:
                    en = block(en)
        ...
        for block in self.blocks_d:
            if args.grad_cp == 1:
                x = deepspeed.checkpointing.checkpoint(block, x,en)
            else:
                x = block(x,en)
        ...
        if self.args.scaffold:
            num =  int(self.args.scaffold_maxlen)
        else:
            num = 0
...
class MyRWKV(pl.LightningModule):
    ...
    def forward(self, samples):
        ...
        x,p,scaffold, y = samples["x"], samples["prop"], samples["sca_tensor"], samples["y"]
        x = self.rwkv(x,p,scaffold) # 
        return x, y
    ...
    @torch.no_grad()
    def predict(self, input_points,p=None,sca=None) -> list[int]:
        
        outputs = self.rwkv(input_points,p,sca)
        return outputs
